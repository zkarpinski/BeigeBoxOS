...
## 2026-07-20 - [MEDIUM] HTML Sanitizer Attribute Bypass

**Vulnerability:** The HTML sanitizer iterated over `el.attributes` while simultaneously removing malicious attributes.
**Learning:** `el.attributes` is a live `NamedNodeMap`. Removing an attribute at index `i` causes the next attribute to shift into index `i`, meaning the loop might skip the subsequent attribute if iterating forward.
**Prevention:** Always convert live collections to a static array using `Array.from(el.attributes)` before iterating and modifying them to ensure every item is processed.
