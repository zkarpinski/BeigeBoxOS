...
## 2026-06-25 - [HIGH] HTML Sanitizer Bypass via Live Collections
**Vulnerability:** The Word 97 HTML sanitizer iterated over `el.attributes` while removing malicious attributes. Since `NamedNodeMap` is a live collection, removing an attribute causes the collection to re-index, leading the loop to skip the next attribute.
**Learning:** Mutating a live DOM collection during iteration is a classic bypass technique. Even a reverse loop can be confusing or brittle if the implementation isn't extremely careful.
**Prevention:** Always convert live DOM collections to static arrays (e.g., `Array.from(el.attributes)`) before iterating and performing deletions. This ensures every element is inspected regardless of mutations to the underlying DOM.
