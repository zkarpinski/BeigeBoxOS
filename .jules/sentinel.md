...
## 2026-07-22 - [HIGH] HTML Sanitizer bypasses via live collection mutation

**Vulnerability:** The Word 97 HTML sanitizer was vulnerable to attribute-stripping bypasses because it iterated over a live `NamedNodeMap` while modifying it. Additionally, it lacked coverage for several dangerous tags (`form`, `video`, etc.), protocols (`vbscript:`), and CSS properties (`behavior:`, `-moz-binding:`).
**Learning:** When removing attributes from a DOM element in a loop, always convert the collection to a static array (e.g., `Array.from(el.attributes)`) first. This prevents subsequent attributes from being skipped due to the live collection's re-indexing.
**Prevention:** Use `Array.from()` for robust iteration over DOM collections that will be mutated. Maintain an expansive list of dangerous tags and legacy browser-specific CSS properties in sanitizers.
