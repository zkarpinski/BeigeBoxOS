...
## 2026-07-22 - [HIGH] Bypassing attribute sanitization via live collection mutation

**Vulnerability:** HTML sanitizers that iterate through the live `element.attributes` collection while removing malicious ones can be bypassed because the indices shift, causing the loop to skip the attribute immediately following a removed one.
**Learning:** Domesticating a live `NamedNodeMap` requires careful iteration. Forward loops skip items; even reverse loops can be fragile if logic grows complex.
**Prevention:** Always clone live DOM collections into a static array (e.g., `Array.from(el.attributes)`) before iterating if the loop body involves removing or replacing attributes.
