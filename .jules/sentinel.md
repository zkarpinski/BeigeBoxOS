...
## 2026-07-22 - [HIGH] Sanitizer bypass via attribute mutation

**Vulnerability:** The Word 97 HTML sanitizer was susceptible to bypasses because it iterated over a live `NamedNodeMap` of attributes while removing them, causing some malicious attributes to be skipped.
**Learning:** Removing attributes from a live collection during forward iteration shifts the remaining attributes, which can lead to subsequent dangerous attributes being missed by the loop.
**Prevention:** Always convert live collections like `el.attributes` to a static array using `Array.from()` before iterating over them if you intend to modify the collection (e.g., by removing attributes).
