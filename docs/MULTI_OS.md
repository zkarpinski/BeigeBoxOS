...
1. **`OsShellProvider`** — Each OS `Desktop` passes its native `AppWindow`, `TitleBar`, `MenuBar`, and `writeFile` (virtual FS). Shared windows call `useOsShell()` so the **window shell** (title bar, borders, menu bar, controls) matches the running OS (Win98 vs XP Luna vs KarpOS).
...
4. **Contracts** — `AppWindowProps`, `TitleBarProps`, `MenuBarProps` / `MenuItemConfig`, and `OsShellValue` live in `@retro-web/core/types/os-shell`. OS implementations re-export these types for local imports.
...
- Implement `AppWindow` + `TitleBar` + `MenuBar` for that shell (or reuse from an existing OS if the chrome is identical).
