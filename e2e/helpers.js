...
const { getAppByWindowId } = require('./apps/apps-config');
 ...
 * Windows97.hideApp if the window is still visible after 500ms.
 * @param {import('@playwright/test').Page} page
...
async function closeAppWindow(page, windowId) {
  ...
  if (stillVisible) {
    await page.evaluate((id) => {
      if (window.Windows97) window.Windows97.hideApp(id);
    }, appId);
  }
}
