...
export function sanitizeHTML(html: string): string {
  ...
  doc.querySelectorAll('*').forEach((el) => {
    // Use Array.from to ensure we iterate over a static collection while mutating attributes
    Array.from(el.attributes).forEach((attr) => {
      const attrName = attr.name.toLowerCase();
      // Normalize value: lowercase, remove whitespace, and strip backslashes (often used to bypass filters)
