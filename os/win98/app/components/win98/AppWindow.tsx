...
export function AppWindow({
  ...
  // Drag, resize grip, focus, and setBounds are all handled by the hook.
  ...
  // Stable ref so the chrome effect can always call the latest minimizeApp.
  const minimizeAppRef = useRef(minimizeApp);
  minimizeAppRef.current = minimizeApp;

  // Saved pre-maximize bounds for restore.
  ...
  // Drag and resize are handled by useWindowBehavior above.
  useEffect(() => {
    const el = windowRef.current;
    if (!el) return;
